...
const STATS: {
  ...
  { key: "followers", label: "Followers", icon: "group" },
  { key: "impressions28d", label: "Impressions (28d)", icon: "visibility" },
  { key: "engagedUsers28d", label: "Engaged (28d)", icon: "auto_awesome" },
  { key: "newFollowers28d", label: "New followers (28d)", icon: "person_add" },
];
        ...
        {STATS.map((stat, i) => (
          ...
          >
            <span className="flex items-center gap-1.5 text-[11px]" style={{ color: "var(--ink-soft)" }}>
              <Icon name={stat.icon} className="h-3 w-3" /> {stat.label}
            </span>
...
function EmptyState({ text }: { text: string }) {
  return (
    ...
    >
      <Icon name="thumb_up" className="h-5 w-5" style={{ color: "var(--ink-soft)" }} />
