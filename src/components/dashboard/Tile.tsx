...
export function Tile({ title, icon, size = "md", index = 0, children, action, active = false }: {
  ...
  return (
    <motion.section
      ...
      transition={{ layout: { type: "spring", stiffness: 300, damping: 30 }, default: { duration: 0.45, delay: index * 0.06, ease: [0.22, 1, 0.36, 1] } }}
    ...
    >
      <header className="relative z-10 mb-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold tracking-wide" style={{ color: "var(--ink)" }}>{title}</h2>
